...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE
        weight = tf.Variable(self.pretrained_embeddings)
        
        wv = tf.nn.embedding_lookup(weight,self.input_placeholder)
        embeddings = tf.reshape(wv,[-1,self.config.n_features*self.config.embed_size])
        ### END YOUR CODE
        return embeddings
