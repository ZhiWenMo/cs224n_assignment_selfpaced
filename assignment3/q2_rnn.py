...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        pad_labels = labels[:max_length]
        mask = len(pad_sentence)*[True]
        
        while len(pad_sentence)<max_length:
            pad_sentence.append(zero_vector)
            pad_labels.append(zero_label)
            mask.append(False)
            
        ret.append((pad_sentence,pad_labels,mask))
        ### END YOUR CODE ###
    return ret
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        wv = tf.nn.embedding_lookup(embed,self.input_placeholder)
        embeddings = tf.reshape(wv,
    ...
    def add_prediction_op(self):
        ...
        b2 = tf.get_variable(initializer=tf.contrib.layers.xavier_initializer(),
                           ...
                           name='b2')
        h_t = tf.zeros((tf.shape(x)[0], self.config.hidden_size), name="h")
        ### END YOUR CODE
        ...
        preds = tf.stack(preds)#shape=[T,batch_size,n_classes]
