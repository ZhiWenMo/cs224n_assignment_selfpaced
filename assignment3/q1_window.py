...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        for i in range(window_size,len(sentence)+window_size):
            window = []
            for ii in range(i-window_size, i+window_size+1):
                window += pad_sentence[ii]
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (!3-5 lines)
        weights = tf.Variable(self.pretrained_embeddings)
        wv = tf.nn.embedding_lookup(weights,self.input_placeholder)
        embeddings = tf.reshape(wv,[-1,self.config.n_window_features*self.config.embed_size])
    ...
    def add_prediction_op(self):
        ...
        W = tf.get_variable(initializer=tf.contrib.layers.xavier_initializer(), shape=[self.config.n_window_features * self.config.embed_size, self.config.hidden_size], name="W")
        b1 = tf.Variable(tf.zeros(self.config.hidden_size), name="b1")
        U = tf.get_variable(initializer=tf.contrib.layers.xavier_initializer(), shape=[self.config.hidden_size, self.config.n_classes], name="U")
